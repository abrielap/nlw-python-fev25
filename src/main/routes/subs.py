...
def get_subscribers_by_link(link, event_id):  # Renomeei a função para evitar conflito
    subs_repo = SubscribersRepository()
...
def get_event_ranking(event_id):  # Renomeei a função para evitar conflito
    subs_repo = SubscribersRepository()
    subs_manager = SubscriberManager(subs_repo)

    http_request = HttpRequest(param={"event_id": event_id})  # Removi o "link", pois não existe aqui
